...
const BlogItem = ({blogData}: BlogItemProps) => {
    ...
    const imageStyles = {width: '100%', height: 200, borderBottomLeftRadius: 6, borderTopLeftRadius: 6}
    return (
        <Paper elevation={3}
               sx={{
                   position: 'relative',
                    ...
                    <Box>
                        ...
                        {blogData.id === 4 &&
                            <Image src={blogFourImage} alt={'Pic'} width={IMAGE_WIDTH} height={IMAGE_HEIGHT}
                                   style={imageStyles}/>}
                        {blogData.id === 5 &&
                            <Image src={blogImageFive} alt={'Pic'} width={IMAGE_WIDTH} height={IMAGE_HEIGHT}
                                   style={imageStyles}/>}
                    </Box>
                </Grid>
                <Grid item xs={12} sm={12} md={6} lg={6} p={1}>
                    <Stack alignItems={'center'}>
                        <EuclidText textAlign={'center'} text={attributes.title.toLowerCase()}
                                    sx={{fontWeight: 700,my:0.5, fontSize: 14, textTransform: 'capitalize !important'}}/>
                        <Box>
